...
每轮扫描必须由用户主动触发；搜索扫描只读取启动时所在标签页的当前结果页，不点击职位卡片或分页控件，也不会自动处理后续页。项目不会代表用户投递职位、发送消息或操作联系方式；用户判断、备注和投递状态都由用户手动记录。

## 功能

- 检测 BOSS 搜索列表、搜索详情组合页、单职位详情页、登录页、挑战页和不支持的布局。
- 搜索页每轮只处理当前页；兼容字段 `maxPages` 只接受 `1`，其他值会在开始前明确拒绝。
- 列表阶段只根据标题、公司、已有地点/薪资和去重信息预筛；读取完整 JD 后再做详情硬规则筛选。
- 搜索页详情只使用 content session 内的原始定位 URL 做同源 direct fetch，不点击或读取右侧实时面板兜底。只有 Job ID 权威匹配且标题/公司没有明确冲突时才允许保存；标题和公司单独一致不能通过。每次真实 fetch（包括唯一一次临时网络 retry）都经过并发 1、1800ms 基础间隔、最多 ±20% 抖动、取消和绝对轮次截止时间 gate。
...
- **unsupported layout**：单个 direct fetch 返回动态壳、缺详情容器或缺字段时只跳过该职位，不会点击卡片兜底。只有列表整体无法解析，或详情至少 8 个样本中同类 Parser 错误达到 75%，才停止整轮。仍失败时需要用脱敏且经过审查的真实证据更新 `packages/boss-adapter` 的 fixture、Selector 和测试。
- **身份校验失败**：搜索扫描必须由卡片 Job ID 或 canonical 详情 URL pathname 中的 Job ID 精确匹配；标题和公司只能检查冲突，不能单独确认身份。失败职位拒绝保存和 AI，随后继续下一职位。独立详情页保留原有兼容校验。
- **diagnostics 写入失败**：Side Panel 会显示警告；职位保存、筛选和已完成评估的状态保持不变。

## 当前限制

- 当前版本只支持 BOSS 直聘网页版。
- 搜索扫描只覆盖当前页；如需处理下一页，必须由用户自行导航后开始新一轮。
- 当前没有经过审查的 `fixtures/boss/network/` 证据，因此未实现 MAIN-world network observer、模板学习或模板重放；当前安全基线只使用受控 direct fetch，不猜测 BOSS 接口或响应字段。
