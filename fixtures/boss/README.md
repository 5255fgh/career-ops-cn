...
覆盖范围：搜索列表、只有 ID/标题/公司/详情 URL 的最小列表卡片、独立详情、直接 fetch 只返回动态壳、搜索详情面板、整张卡片为链接的安全激活、公司职位列表、字段缺失、登录、挑战、账号异常、空页面、未知布局、点击 A 后详情仍为 B、详情内容未变化，以及无 `sourceJobId` 的身份验证。
